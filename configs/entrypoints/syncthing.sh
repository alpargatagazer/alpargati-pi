...
# - Configures "Microservices Volumes" and "Public Share" folders
# ============================================================================
...
# Specific folders configuration
...
# Ensure paths exist
mkdir -p "$VOLUMES_PATH" "$SHARE_PATH"

# Setup .stignore
setup_stignore "$VOLUMES_PATH"
setup_stignore "$SHARE_PATH"

# Generate unique folder IDs
VOLUMES_FOLDER_ID=$(head -c 20 /dev/urandom | od -An -tx1 | tr -d ' \n')
SHARE_FOLDER_ID=$(head -c 20 /dev/urandom | od -An -tx1 | tr -d ' \n')

# Add folders to config
add_folder "$VOLUMES_FOLDER_ID" "$VOLUMES_LABEL" "$VOLUMES_PATH"
add_folder "$SHARE_FOLDER_ID" "$SHARE_LABEL" "$SHARE_PATH"

# Ensure versioning is enabled for existing folders
ensure_versioning "$VOLUMES_PATH"
ensure_versioning "$SHARE_PATH"
...
exec syncthing --home "$CONFIG_HOME" --gui-address="0.0.0.0:8384"
